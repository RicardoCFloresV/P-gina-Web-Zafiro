...
async function request(url, options = {}) {
  ...
  try {
    ...
    console.error(`[unidadesApi] Error de red en ${url}:`, netErr);
  ...
  try {
    ...
    console.error(`[unidadesApi] Respuesta no-JSON en ${url} (status ${res.status}):`, parseErr);
    throw new Error(NO_SERVER);
  }
  if (!res.ok) {
    const msg = json.message || `Error HTTP ${res.status}`;
    console.warn(`[unidadesApi] Error del servidor en ${url}:`, msg);
    throw new Error(msg);
  }
  return json;
}
...
const unidadesApi = {
  ...
  async insert({ nombre }) {
    const body = { nombre };
    console.log('[unidadesApi.insert] Datos enviados a /api/unidades/insert:', body);
  ...
  async update({ unidad_id, nombre }) {
    ...
    console.log('[unidadesApi.update] Datos enviados a /api/unidades/update:', body);
  ...
  async setState({ unidad_id, estado }) {
    ...
    console.log('[unidadesApi.setState] Datos enviados a /api/unidades/set_state:', body);
